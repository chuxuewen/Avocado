...
ParserBase<Traits>::ParsePropertyDefinition(
  ...
  if (/*!in_class && */!is_generator) {
    ...
    if (token == Token::COLON) {
      ...
      if ( in_class ) {
        if (peek() != Token::RBRACE && ! scanner()->HasAnyLineTerminatorBeforeNext()) {
          if (scanner()->current_token() != Token::RBRACE) {
            // Need {} because of the CHECK_OK macro.
            Expect(Token::SEMICOLON, CHECK_OK_CUSTOM(EmptyObjectLiteralProperty));
          }
        }
      }
    ...
    if (!in_class && Token::IsIdentifier(name_token, language_mode(), this->is_generator(),
                                         parsing_module_) &&
...
void ParserBase<Traits>::ClassLiteralChecker::CheckProperty(
    Token::Value property, PropertyKind type, MethodKind method_type,
    bool* ok) {
  DCHECK(type == kMethodProperty || type == kAccessorProperty || type == kValueProperty);
  ...
  if (IsStaticMethod(method_type)) {
    ...
    if (is_generator || is_async || type == kAccessorProperty || type == kValueProperty) {
    ...
    if ( type == kValueProperty ) {
      this->parser()->ReportMessage(MessageTemplate::kNotPropertyName);
      *ok = false;
    }
  }
  //
